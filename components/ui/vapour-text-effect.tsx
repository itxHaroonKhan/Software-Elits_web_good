...
export const Component = () => (
  <div className="bg-black h-screen w-screen flex justify-center items-center">
    <VaporizeTextCycle
      texts={["Software", "Elites", "Agency"]}
      font={{ fontFamily: "Inter, sans-serif", fontSize: "70px", fontWeight: 700 }}
...
export default function VaporizeTextCycle({
  ...
  const fontConfig = useMemo(() => {
    ...
    return {
      fontSize, VAPORIZE_SPREAD, MULTIPLIED_VAPORIZE_SPREAD,
  ...
  const memoizedUpdateParticles = useCallback((particles: Particle[], vaporizeX: number, deltaTime: number) =>
    updateParticles(particles, vaporizeX, deltaTime, fontConfig.MULTIPLIED_VAPORIZE_SPREAD,
...
/* ── Helpers ── */
function handleFontChange({ currentFont, lastFontRef, canvasRef, wrapperSize, particlesRef, globalDpr, currentTextIndex, transformedDensity, framerProps }: any) {
  if (currentFont !== lastFontRef.current) {
    lastFontRef.current = currentFont;
    const t = setTimeout(() => { cleanup({ canvasRef, particlesRef }); renderCanvas({ framerProps, canvasRef, wrapperSize, particlesRef, globalDpr, currentTextIndex, transformedDensity }); }, 1000);
...
function renderCanvas({ framerProps, canvasRef, wrapperSize, particlesRef, globalDpr, currentTextIndex, transformedDensity }: any) {
  ...
  const { particles, textBoundaries } = createParticles(ctx, canvas, currentText, textX, textY, font, color, align);
...
function updateParticles(particles: Particle[], vaporizeX: number, deltaTime: number, SPREAD: number, VAPORIZE_DURATION: number, direction: string, density: number) {
  ...
  particles.forEach(p => {
    ...
    if (shouldVaporize) {
      ...
      if (p.shouldFadeQuickly) {
        ...
        const mv = SPREAD * 2, cv = Math.sqrt(p.velocityX ** 2 + p.velocityY ** 2);
